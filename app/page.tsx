...
const REGION_GROUPS = [
  ...
  },
  {
    ...
    regions: [
      ...
      { code: 'MEA', name: 'Middle East & Africa', flag: '🌍' },
    ]
  }
];
...
export default function ListingLensHome() {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (ddRef.current && !ddRef.current.contains(e.target as Node)) setShowRegions(false);
    };
  ...
  const onFile = (f: File) => {
    ...
    r.onload = (e) => { setImg(e.target?.result as string); setFile(f); };
    r.readAsDataURL(f);
  };

  const clear = () => { setImg(null); setFile(null); if (fileRef.current) fileRef.current.value = ''; };

  const analyze = async () => {
    if (!file || !cat) return;
    setBusy(true); setStep('Reading your listing...');
    try {
  ...
  const blue = '#2563eb';
  const dimText = dark ? '#52525b' : '#d6d3d1';

  const btnStyle = (selected: boolean): React.CSSProperties => ({
    padding: 16, borderRadius: 12, border: `2px solid ${selected ? blue : catBrd}`,
    backgroundColor: selected ? blue : cardBg, color: selected ? '#fff' : text,
    cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center',
    gap: 8, transition: 'all 0.2s', fontSize: 13, fontWeight: 700,
  });
    ...
    <div style={{ minHeight: '100vh', backgroundColor: bg, color: text, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', transition: 'all 0.3s' }}>
      ...
      <main style={{ maxWidth: 800, margin: '0 auto', padding: '32px 16px' }}>
        ...
        <section style={{ textAlign: 'center', marginBottom: 40 }}>
          <h2 style={{ fontSize: 52, fontWeight: 900, letterSpacing: '-0.03em', margin: 0, lineHeight: 1 }}>DON'T BUY</h2>
          <h2 style={{ fontSize: 52, fontWeight: 900, letterSpacing: '-0.03em', margin: '0 0 24px', lineHeight: 1 }}>BLIND<span style={{ color: '#3b82f6' }}>.</span></h2>
          <p style={{ fontSize: 18, fontWeight: 500, color: muted, marginBottom: 16 }}>Your Personal Online Buyer's Advocate</p>
        ...
        <section style={{ marginBottom: 32 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <div style={{ position: 'relative' }} ref={ddRef}>
              <button onClick={() => setShowRegions(!showRegions)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 20px', borderRadius: 10, backgroundColor: cardBg, border: `1px solid ${border}`, color: text, cursor: 'pointer', fontSize: 15, fontWeight: 500 }}>
                <span style={{ fontSize: 22 }}>{region.flag}</span>
                <span>{region.name}</span>
                <ChevronDownIcon />
              </button>
              {showRegions && (
                <div style={{ position: 'absolute', top: '100%', marginTop: 8, left: '50%', transform: 'translateX(-50%)', backgroundColor: cardBg, border: `1px solid ${border}`, borderRadius: 12, boxShadow: '0 10px 40px rgba(0,0,0,0.25)', overflow: 'hidden', zIndex: 50, minWidth: 240 }}>
                  {REGION_GROUPS.map((group) => (
                    <div key={group.label}>
                      ...
                      {group.regions.map((r) => (
                        <button key={r.code} onClick={() => { setRegion(r); setShowRegions(false); }}
                          ...
                          <span style={{ fontSize: 20 }}>{r.flag}</span><span>{r.name}</span>
                        </button>
                      ))}
                    </div>
                  ))}
            ...
            {CATEGORIES.map((c) => {
              ...
              return (
                <button key={c.id} onClick={() => setCat(c.id)} style={btnStyle(sel)}>
                  <c.Icon size={24} /><span>{c.label}</span>
                </button>
              );
            })}
          </div>
            ...
            <div onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); const f = e.dataTransfer.files?.[0]; if (f) onFile(f); }}
              ...
              {img ? (
                ...
                <>
                  ...
                  <input ref={fileRef} type="file" accept="image/*" onChange={(e) => { const f = e.target.files?.[0]; if (f) onFile(f); }}
        ...
        {img && cat && (
          <section style={{ marginBottom: 32 }}>
            <button onClick={analyze} disabled={busy}
              style={{ width: '100%', padding: 16, borderRadius: 12, border: 'none', backgroundColor: busy ? '#52525b' : blue, color: '#fff', fontSize: 18, fontWeight: 700, cursor: busy ? 'not-allowed' : 'pointer', transition: 'all 0.2s' }}>
              {busy ? step : 'ANALYZE THIS LISTING →'}
            </button>
            <p style={{ textAlign: 'center', fontSize: 14, marginTop: 12, color: muted }}>$4.99 AUD · Full report in under 60 seconds</p>
          </section>
        )}
        ...
        <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, padding: '32px 0', borderTop: `1px solid ${border}` }}>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 12, fontWeight: 600, margin: 0 }}>150+ Countries</p>
            <p style={{ fontSize: 12, color: muted, margin: '4px 0 0' }}>Global coverage</p>
          </div>
        </section>
      </main>
        ...
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '0 16px' }}>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 24, fontSize: 14 }}>
            {['Privacy', 'Terms', 'Contact', 'API'].map(l => (
              <a key={l} href={`/${l.toLowerCase()}`} style={{ color: muted, textDecoration: 'none' }}>{l}</a>
            ))}
          </div>
      ...
      {showModal && (
        <div onClick={() => setShowModal(false)} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ backgroundColor: cardBg, borderRadius: 16, maxWidth: 420, width: '100%', padding: 24, border: `1px solid ${border