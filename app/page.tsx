...
export default function ListingLens() {
  ...
  useEffect(() => {
    ...
    if (cancelled) {
      // User cancelled payment - restore what we can
      ...
      if (savedCategory) {
        ...
        // Go back to step 1 - they'll need to re-upload since blob may expire
        setStep(1);
    ...
    if (sessionId) {
      ...
      setRegion(savedRegion || 'AU');
      setRegionFlag(savedRegionFlag || '🇦🇺');
      setStep(5); // Go to loading state
      setIsProcessing(true);
      setProcessingMessage('Payment verified. Generating your full report...');

      // Generate the full report (image is in Netlify Blobs, not localStorage)
  ...
  const generateTeaser = async (file: File) => {
    ...
    try {
      ...
      // Store in localStorage as backup (in case Stripe redirect has issues)
  ...
  const generateFullReport = async (sessionId: string) => {
    try {
      // Image is stored in Netlify Blobs - we just need the session_id
